...
onRecordCreateRequest((e) => {
  ...
  console.log(
    `DEBUG: Message Create Request - ID: ${e.record.id || "new"} - Starting processing`,
  );

  try {
    // Log request details
    const requestInfo = e.requestInfo();
    const contentType = requestInfo.headers["content_type"] || "none";
    console.log(`DEBUG: Message create request details:
      ...
      Content Type: ${requestInfo.headers["content_type"] || "none"}
    `);
    ...
    console.log(
      `DEBUG: Message data - ChatID: ${chatId}, Sender: ${sender}, Current content: "${e.record.get("content") || ""}"`,
    );
    ...
    if (contentType.includes("form-data")) {
      ...
      console.log(
        `DEBUG: Image files found: ${hasImage}, Count: ${imageFiles?.length || 0}`,
      );
    } else {
      console.log(
        `DEBUG: Content type is not multipart/form-data, skipping image check`,
      );
    }
    // Set message type based on content
    if (hasImage) {
      console.log(
        `DEBUG: Processing image message - Setting messageType to "image"`,
      );
      ...
      console.log(
        `DEBUG: Proceeding with image: ${imageFile.name}, Size: ${(imageFile.size / 1024).toFixed(2)}KB, Type: ${imageFile.type}`,
      );
    ...
    console.log(
      `DEBUG: Message create request processing completed successfully`,
    );
  } catch (err) {
...
onRecordAfterCreateSuccess((e) => {
  ...
  console.log(
    `DEBUG: Message After Create Success - ID: ${e.record.id} - Starting post-processing`,
  );
  ...
  console.log(
    `DEBUG: Processing created message - Type: ${messageType}, ChatID: ${chatId}`,
  );
  ...
  try {
    if (messageType === "image") {
      console.log(
        `DEBUG: Processing image type message - Generating thumbnail`,
      );
      ...
      if (image) {
        ...
        const thumbUrl = `/api/files/messages/${e.record.id}/${image}?thumb=0x300`;
        console.log(`DEBUG: Generated thumbnail URL: ${thumbUrl}`);

        // Update record with thumbnail URL
        console.log(
          `DEBUG: Setting thumbnailUrl field on message ${e.record.id}`,
        );
      ...
      console.log(
        `DEBUG: Non-image message type (${messageType}), skipping thumbnail generation`,
      );
    }
    ...
    if (chatId) {
      ...
      try {
        ...
        console.log(
          `DEBUG: Found chat: ${chat.id} - Current messages: ${(chat.get("messages") || []).length}`,
        );
        console.log(
          `DEBUG: Current chat preview: "${chat.get("lastMessageSnippet") || ""}"`,
        );

        const sender = e.record.get("sender") || "";
        const timestamp = e.record.get("created") || new Date().toISOString();
        console.log(
          `DEBUG: Message sender: ${sender}, timestamp: ${timestamp}`,
        );
        ...
        // Update chat preview
        console.log(
          `DEBUG: Setting chat preview fields - Snippet: "${snippet}"`,
        );
        ...
        console.log(
          `DEBUG: Adding message ${e.record.id} to chat's message list`,
        );
        ...
        console.log(
          `DEBUG: Updated message list: ${originalCount} → ${messages.length} messages`,
        );
        ...
        console.log(
          `DEBUG: Chat save result: ${saveResult ? "Success" : "Failed"}`,
        );
        console.log(
          `DEBUG: Updated chat ${chatId} with new message preview: "${snippet}"`,
        );
      } catch (err) {
...
onRecordUpdateRequest((e) => {
  ...
  // Only process messages with image type
  if (e.record.get("messageType") !== "image") {
    e.next();
    return;
  }

  console.log("hasRequestInfo => ", Object.hasOwn(e, "requestInfo"));

  const requestInfo = e.requestInfo();
  const record = e.record;

  console.log(
    "onRecordAfterUpdateSuccess - record",
    JSON.stringify(record, null, 2),
  );

  if (requestInfo.body["isActive"] === false) {
    console.log(
      `DEBUG: Skipping image message processing for inactive message`,
    );
    e.next();
    return;
  }
...
onRecordAfterUpdateSuccess((e) => {
  ...
  //skip image message processing for message removal requests
  if (e.record.get("isActive") === false) {
    console.log(
      `DEBUG: Skipping image message processing for inactive message`,
    );
    e.next();
