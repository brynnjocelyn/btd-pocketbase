...
onRecordsListRequest((e) => {
  ...
  console.log("DEBUG: Before filtering - Total messages:", e.result?.items?.length || 0);
  if (e.result?.items?.length > 0) {
    console.log("DEBUG: First message isActive value:", e.result.items[0].get("isActive"));
    console.log("DEBUG: First message fields:", JSON.stringify(e.result.items[0].data, null, 2));
  }
  
  // Use the imported filterActiveRecords function
  utils.filterActiveRecords(e.result);
  
  console.log("DEBUG: After filtering - Total messages:", e.result?.items?.length || 0);
  if (e.result?.items?.length > 0) {
    console.log("DEBUG: Sample message after filtering:", JSON.stringify(e.result.items[0].data, null, 2));
    console.log("DEBUG: Request filter:", JSON.stringify(e.requestInfo().query, null, 2));
  } else {
...
onRecordViewRequest((e) => {
  if (e.hasSuperuserAuth()) {
    console.log("DEBUG: View request - Superuser detected, allowing view of message:", e.record.id);
    e.next();
    return;
  }
  ...
  console.log("DEBUG: View request - Checking if message is active:", e.record.id);
  console.log("DEBUG: View request - isActive value:", e.record.get("isActive"), "type:", typeof e.record.get("isActive"));
  console.log("DEBUG: View request - Message fields:", JSON.stringify(e.record.data, null, 2));
...
onRecordAfterCreateSuccess((e) => {
  ...
  if (chat) {
    ...
    utils.updateChatWithMessageDetails(chat, message.id, snippet, timestamp, senderId);
